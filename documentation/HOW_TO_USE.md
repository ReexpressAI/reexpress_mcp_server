...
If we need to further understand the calculation, we can call ReexpressView or look at current_reexpression.html in the model directory. In this example, we see that the prediction-conditional estimate is very high at 0.9936213860453533[^4], which is above the corresponding class threshold of 0.9338333010673523; the effective sample size is 20688; the rescaled q value is 2.2818913309070847 (which is above the cutoff of 1.0003684141164448); and for example, among the underlying signals that went into the calculation, the distance quantile of 0.3010709285736084 is less than the median (values closer to 1 indicate closer distances to training), but in this case, is sufficiently close to the observed training data to still obtain a reliable estimate at the alpha' value of 0.9.
