...
The "Admitted" instances are those for which the predicted probability from the SDM estimator for the verified class (i.e., label 1) is in the `High-Reliability` region. See the paper ["Similarity-Distance-Magnitude Activations"](https://arxiv.org/abs/2509.12760) for details. In the context of the MCP Server, these instances would have a confidence `>= 90%` in the main output.
