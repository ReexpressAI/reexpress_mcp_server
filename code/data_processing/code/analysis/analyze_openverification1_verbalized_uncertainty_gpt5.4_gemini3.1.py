...
JOINT_MODEL_LABEL_KEY = "gpt-5.4-2026-03-05_and_gemini-3.1-pro-preview"
...
def construct_agreement_template(row, is_eval=False):
    ...
    label = int(row['label'])
    summary_from_previous_gpt_model = row['model5_short_summary_of_original_question_and_response'].strip()
    summary_from_latest_gpt_model = row['model7_short_summary_of_original_question_and_response'].strip()
    assert label in [0, 1]
    ...
    previous_gpt_model_explanation = row["model5_short_explanation_for_classification_confidence"]

    latest_gemini_model_explanation = row["model8_short_explanation_for_classification_confidence"]
    latest_gpt_model_explanation = row["model7_short_explanation_for_classification_confidence"]
    ...
    if gpt_and_gemini_present:
        if latest_gpt_present:
            ...
            latest_gpt_prediction = int(row['model7_verification_classification'])
            latest_gpt_confidence = float(row['model7_confidence_in_classification'])
            ...
            gpt_attributes = get_confidence_soft_one_hot_list(row['model5_verification_classification'],
...
def main():
    ...
    print("For this script, we give verbalized uncertainty the benefit of treating API rejections as outside the "
          "admitted set, whereas for the SDM estimator, they are treated as wrong predictions.")
