...
def test(options, id2ensemble_stats=None, numberOfClasses=2, maxQAvailableFromIndexer=2048, total_models_in_ensemble=0,
    ...
    assert most_conservative_hr_alpha_to_consider_in_ensemble is not None, \
        f"ERROR: You need to specify the alpha value of the most conservative region under consideration. " \
        f"We recommend using that of the model iteration with the lowest held-out balanced SDM loss."
    print(f"###############Beginning of Ensemble evaluation ({total_models_in_ensemble} models)###############")

    alpha_prime = most_conservative_hr_alpha_to_consider_in_ensemble
    print(f'\tMost conservative alpha to determine the high reliability region: {alpha_prime}')
    assert alpha_prime > 0.5, \
        "The main model has no recorded high-reliability regions (or an invalid alpha); " \
        "the ensemble HR criteria are undefined. Retrain/recalibrate before ensembling."
    ...
    for document_id in id2ensemble_stats:
        ...
        is_ood = False  # old definition for reference
        ...
        for prediction_meta_data in prediction_meta_data_across_models:
            ...
            # We choose the min regions and probabilities among the ensembles members with the chosen modal class:
